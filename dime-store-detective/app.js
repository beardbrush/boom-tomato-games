// app.js — Dime Store Detective (safe boot + safe JSON loader)
// Fixes "stuck on Loading case..." by waiting for DOM + avoiding early null crashes.
// Also detects "HTML returned instead of JSON" and falls back cleanly.
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // If core UI isn't present, bail with a clear message
  if (!speakerLabel || !answerEl || !accuseBtn) {
    console.error("[Dime Store Detective] Missing required HTML elements. Check IDs in index.html.");
    alert("Dime Store Detective: missing required UI elements. Check index.html IDs.");
    return;
  }

  // Start label
  speakerLabel.textContent = "DIME STORE DETECTIVE";
  answerEl.textContent = "Loading case...";

  /* ================== SPRITES (GUARDED) ================== */
  const detCtx = detCanvas ? detCanvas.getContext("2d") : null;
  const susCtx = susCanvas ? susCanvas.getContext("2d") : null;
  ...
  /* ================== RENDER LOOP (GUARDED) ================== */
  function mainLoop(t) {
    if (detCtx) drawDetective(t);
    if (susCtx) drawRightSide(t);
  ...
  function drawRightSide(t) {
    ...
    if (viewMode === "wren") {
      ...
      susCtx.drawImage(wrenImg, wrenFrame * frameW, 0, frameW, frameH, dx, dy, destW, destH);
      return;
    }
    ...
    susCtx.clearRect(0, 0, susCanvas.width, susCanvas.height);
    susCtx.drawImage(img, suspectState.frame * frameW, 0, frameW, frameH, dx, dy, destW, destH);
  }
  ...
  questionButtons.forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      if (detCanvas && !detCanvas.classList.contains("visible")) {
  ...
  function renderDirectory() {
    ...
    directoryPeople.forEach(key => {
      ...
      const initials = person.displayName.split(" ").map(w => w[0]).join("").slice(0, 3);
  ...
  /* ================== YOUR SAFE loadCase() (WITH TIMEOUT + HTML GUARD) ================== */
  async function loadCase(index) {
    ...
    try {
      ...
      if (looksLikeHtml || (!ct.includes("application/json") && !trimmed.startsWith("{"))) {
