...
(() => {
  ...
  // ------------------------------
  // PATH HELPERS (fixes case loader)
  // ------------------------------
  function urlFor(rel) {
    return new URL(rel, window.location.href).toString();
  }

  async function safeFetchJson(rel) {
    const u = urlFor(rel);
  ...
  // WebAudio for click taps (no files needed)
  let audioCtx = null;
  ...
  function playClick() {
    ...
    // Some browsers suspend until user gesture; this is always called from clicks
  ...
  async function unlockAudioOnce() {
    ...
    // unlock HTMLAudio for iOS/Android
    try {
      music.muted = true;
      await music.play();
      music.pause();
  ...
  for (const key in suspects) suspects[key].currentAnswers = suspects[key].defaultAnswers.slice();
  ...
  async function loadCharactersMeta() {
    try {
      const data = await safeFetchJson("characters.json");
  ...
  function drawRightSide(t) {
    ...
    const destW = frameW * s.scale;
    const destH = frameH * s.scale;
  ...
  questionButtons.forEach(btn => {
    btn.addEventListener("click", async () => {
      ...
      if (!detCanvas.classList.contains("visible")) detCanvas.classList.add("visible");
  ...
  window.addEventListener("keydown", (e) => {
    ...
    if (accuseOverlay.classList.contains("active")) {
      accuseOverlay.classList.remove("active");
      accuseOverlay.setAttribute("aria-hidden", "true");
      return;
    }
    if (directoryOverlay.classList.contains("active")) {
      directoryOverlay.classList.remove("active");
      directoryOverlay.setAttribute("aria-hidden", "true");
    }
  });
  ...
  function applyCaseToSuspects(caseData) {
    ...
    if (caseData.answers) {
      for (const key in caseData.answers) {
        if (suspects[key]) suspects[key].currentAnswers = caseData.answers[key];
      }
    }
  }
  ...
  function inferActiveSuspects(caseData) {
    ...
    if (caseData.answers) {
      Object.keys(caseData.answers).forEach(k => { if (suspects[k]) inferred.add(k); });
    }
    if (!inferred.size) DEFAULT_CASE.suspects.forEach(k => inferred.add(k));
  ...
  async function loadCase(index) {
    ...
    if (location.protocol === "file:") {
      ...
      answerEl.textContent =
        "This game can't load JSON while opened as a local file.\n\n" +
        "Run it from a local server (or your website) so fetch() can read cases/case1.json.";
      toast("Open via a server, not file://");
      return;
    }

    const rel = `cases/case${index}.json`;

    try {
      const data = await safeFetchJson(rel);
      ...
      console.warn("Case load failed:", rel, err);
      ...
      answerEl.textContent =
        ...
        `Tip: confirm the file exists at:\n${urlFor(rel)}`;

      toast("Case JSON not found");
    }
  }
